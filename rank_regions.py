...
def get_params():
    ...
    parser.add_argument('--impath', type=str, default='/media/Data/datasets/sharp-heatmapts-pt/user_test/')
...
class_dirs = [os.path.join(args.impath, d) for d in dirinfo if os.path.isdir(os.path.join(args.impath, d))]

for class_dir in class_dirs:

    class_name = os.path.basename(class_dir)

    dirinfo = os.listdir(class_dir)
    dirs = [os.path.join(class_dir, d) for d in dirinfo if os.path.isdir(os.path.join(class_dir, d))]
    assert len(dirs) > 0

    print("Found", len(dirs), "folders for class", class_name)
    ...
    for i, d in enumerate(dirs[:nb_img_to_compute]):
        ...
        parent = '/media/Data/datasets/sharp-heatmapts-pt/cropped_regions/user_test/' + class_name
